...
import UserCreteForm, { User } from "./UserCreateForm";
...
export default function CategoriesPage() {
  ...
  async function loadData(query = "", page = 1) {
    setLoading(true);
    try {
      const res = await fetchusers(query, page, 10); // 10 items per page
      setItems(res.data);
...
// }
  ...
  return (
    <div className="p-6">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit User" : "Create New User"}>
        <UserCreteForm
