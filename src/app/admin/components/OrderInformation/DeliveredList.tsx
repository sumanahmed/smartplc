...
import { getOrdersDelivered, updateOrderStatus } from "@/lib/ordersApi";
...
const OrderList = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        setError(null);
        const apiData = await getOrdersDelivered();
  ...
  if (loading) {
    return (
      <p className="text-center text-gray-500 py-6">Loading Delivered orders...</p>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-bold text-gray-900 mb-4">Delivered Order List</h2>
